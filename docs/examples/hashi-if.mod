...
set Eh := { (i,j,k,l) in { V, V }:
        ...
        (sum{ (s,t) in V: s = i and t > j and t < l } 1) = 0             # No vertex inbetween
        };
...
set Ev := { (i,j,k,l) in { V, V }:
        ...
        (sum{ (s,t) in V: t = j and s > i and s < k } 1) = 0             # No vertex inbetween
        };

set E := Eh union Ev;

set EhEvF := setof{(i,j,k,l) in Eh, (s,t,u,v) in Ev:
        s < i and u > i and j < t and l > t}(i,j,k,l,s,t,u,v);
...
s.t. no_crossing1{(i,j,k,l,s,t,u,v) in EhEvF}:
        xe1[i,j,k,l] + xe1[s,t,u,v] <= 1;
s.t. no_crossing2{(i,j,k,l,s,t,u,v) in EhEvF}:
        xe1[i,j,k,l] + xe2[s,t,u,v] <= 1;
s.t. no_crossing3{(i,j,k,l,s,t,u,v) in EhEvF}:
        xe2[i,j,k,l] + xe1[s,t,u,v] <= 1;
s.t. no_crossing4{(i,j,k,l,s,t,u,v) in EhEvF}:
...
set node_target := { (s,t) in V:
        (sum{ (i,j) in V: i < s or (i = s and j < t) } 1) = 0};
...
for { row in rows } {
        for { col in cols } {
                ...
                if givens[row,col] != 0 then printf "%d", givens[row,col];
                else
		{
			if (sum{(i,j,k,l) in Eh: i = row and col >= j and col < l and
                                xe1[i,j,k,l] = 1} 1) = 1 then printf "-";

			if (sum{(i,j,k,l) in Eh: i = row and col >= j and col < l and
                                xe2[i,j,k,l] = 1} 1) = 1 then printf "=";

			if (sum{(i,j,k,l) in Ev: j = col and row >= i and row < k and
                                xe1[i,j,k,l] = 1} 1) = 1 then printf "|";
			...
			if (sum{(i,j,k,l) in Eh: i = row and col >= j and col < l and
                                (xe1[i,j,k,l] = 1 or xe2[i,j,k,l] = 1)} 1) = 0
				and (sum{(i,j,k,l) in Ev: j = col and row >= i and row < k and
                                (xe1[i,j,k,l] = 1 or xe2[i,j,k,l] = 1)} 1) = 0 then printf " ";
		}
                ...
                if (sum{(i,j,k,l) in Eh: i = row and col >= j and col < l} 1) = 0 then printf " ";
        }
        printf "\n";
        for { col in cols } {
                ...
                if (sum{(i,j,k,l) in Ev: j = col and row >= i and row < k} 1) = 0 then printf " ";
                printf " ";
        }
        printf "\n";
}

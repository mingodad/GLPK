...
#model "cut-fix-size-m1.mod";
#model "cut-fix-size-m2.mod";
...
#for {1..4} {
   display nPAT, nPAT_top;
   printf "nbr:";printf{i in WIDTHS, j in PATTERNS : j <= nPAT_top and nbr[i,j] > 0}: "[%d,%d]%g,", i,j, nbr[i,j];printf "\n";
   ...
   #display Number, Reduced_Cost, Fill;
   #display{i in WIDTHS} Fill[i].dual;
   printf "Fill: ";printf{i in WIDTHS} "[%d]%g,", i, Fill[i]; printf "\n";
   printf "Fill.dual: ";printf{i in WIDTHS} "[%d]%g,", i, Fill[i].dual; printf "\n";
   printf "price: ";printf{i in WIDTHS} "[%d]%g,", i, price[i]; printf "\n";
   if Reduced_Cost < -0.00001 and nPAT_top < nPAT then
   {
      let nPAT_top := nPAT_top + 1;
      let {i in WIDTHS} nbr[i,nPAT_top] := Use[i];
      printf "Use: ";printf{i in WIDTHS: Use[i] > 0} "[%d]%g,", i, Use[i]; printf "\n";
      #display nPAT;
   }
   else break;
}
...
if nPAT_top < nPAT then
{
	...
	display Number, nPAT, nPAT_top, Reduced_Cost;

...
#ifdef SQLITE_DLNAME
#define HAVE_SQLITE
#define libsqlite SQLITE_DLNAME
#define h_sqlite (get_env_ptr()->h_sqlite)
#endif
...
#if defined(HAVE_ODBC) || defined(HAVE_MYSQL) || defined(HAVE_SQLITE)
...
{     xassert(dca == dca);
      xassert(mode == mode);
      xprintf("SQLite table driver not supported\n");
      return NULL;
}
...
struct db_sqlite
{
   ...
   /* ref[k] = k', if k-th field of the csv file corresponds to
      ...
      of the csv file is ignored */
   char            *query;
   /* query generated by db_sqlite_open */
};

typedef int dl_sqlite3_open(const char *filename, sqlite3 **ppDb);
typedef SQLITE_API int dl_sqlite3_close(sqlite3*);
typedef const char *dl_sqlite3_errmsg(sqlite3 *db);
typedef const char *dl_sqlite3_errstr(int);
typedef int dl_sqlite3_prepare(
...
typedef int dl_sqlite3_reset(sqlite3_stmt *pStmt);
typedef int dl_sqlite3_finalize(sqlite3_stmt *pStmt);
typedef int dl_sqlite3_column_count(sqlite3_stmt *pStmt);
typedef const char *dl_sqlite3_column_name(sqlite3_stmt*, int N);
typedef int dl_sqlite3_column_type(sqlite3_stmt*, int iCol);
typedef double dl_sqlite3_column_double(sqlite3_stmt*, int iCol);
typedef int dl_sqlite3_column_int(sqlite3_stmt*, int iCol);
typedef sqlite3_int64 dl_sqlite3_column_int64(sqlite3_stmt*, int iCol);
typedef const unsigned char *dl_sqlite3_column_text(sqlite3_stmt*, int iCol);
typedef int dl_sqlite3_step(sqlite3_stmt*);
typedef int dl_sqlite3_bind_double(sqlite3_stmt*, int, double);
typedef int dl_sqlite3_bind_int(sqlite3_stmt*, int, int);
typedef int dl_sqlite3_bind_int64(sqlite3_stmt*, int, sqlite3_int64);
typedef int dl_sqlite3_bind_null(sqlite3_stmt*, int);
typedef int dl_sqlite3_bind_text(sqlite3_stmt*,int,const char*,int,void(*)(void*));
typedef int dl_sqlite3_exec(
...
static struct {
    dl_sqlite3_open *open;
    dl_sqlite3_close *close;
    dl_sqlite3_errmsg *errmsg;
    dl_sqlite3_errstr *errstr;
    dl_sqlite3_exec *exec;
    dl_sqlite3_prepare *prepare;
    dl_sqlite3_reset *reset;
    dl_sqlite3_finalize *finalize;
    dl_sqlite3_column_count *column_count;
    dl_sqlite3_column_name *column_name;
    dl_sqlite3_column_type *column_type;
    dl_sqlite3_column_double *column_double;
    dl_sqlite3_column_int *column_int;
    dl_sqlite3_column_int64 *column_int64;
    dl_sqlite3_column_text *column_text;
    dl_sqlite3_step *step;
    dl_sqlite3_bind_double *bind_double;
    dl_sqlite3_bind_int *bind_int;
    dl_sqlite3_bind_int64 *bind_int64;
    dl_sqlite3_bind_null *bind_null;
    dl_sqlite3_bind_text *bind_text;
} sqlite3_dlfuncs;

static void *db_sqlite_open_int(TABDCA *dca, int mode, const char
   **sqllines)
{
   struct db_sqlite *sql = NULL;
   char            *arg = NULL;
   const char      *field;
   char            *keyword;
   char            *value;
   ...
   if (libsqlite == NULL)
   {
      xprintf("No loader for shared SQLite library available\n");
      return NULL;
   }

   if (h_sqlite == NULL)
   {
      h_sqlite = xdlopen(libsqlite);
      if (h_sqlite == NULL)
      {  xprintf("unable to open library %s\n", libsqlite);
         xprintf("%s\n", get_err_msg());
         return NULL;
...
#define DLSQLITE3(fn) \
    sqlite3_dlfuncs.fn = (dl_sqlite3_##fn *) xdlsym(h_sqlite, "sqlite3_" #fn);\
    if(sqlite3_dlfuncs.fn == NULL) {errmsg = "sqlite3_" #fn; break;}
      errmsg = NULL;
      while(1)
      {
        DLSQLITE3(open);
        ...
        DLSQLITE3(reset);
        DLSQLITE3(finalize);
        DLSQLITE3(column_count);
        DLSQLITE3(column_name);
        DLSQLITE3(column_type);
        DLSQLITE3(column_double);
        DLSQLITE3(column_int);
        DLSQLITE3(column_int64);
        DLSQLITE3(column_text);
        DLSQLITE3(step);
        DLSQLITE3(bind_double);
        DLSQLITE3(bind_int);
        DLSQLITE3(bind_int64);
        DLSQLITE3(bind_null);
        DLSQLITE3(bind_text);
        break;
      }
      if(errmsg)
      {
         xprintf("unable to load function %s in library %s\n", errmsg, libsqlite);
         h_sqlite = NULL;
         return NULL;
      }
#undef DLSQLITE3
   ...
   sql = (struct db_sqlite *) xmalloc(sizeof(struct db_sqlite));
   if (sql == NULL)
         return NULL;
   sql->mode = mode;
   sql->stmt = NULL;
   ...
   narg = mpl_tab_num_args(dca);
   if (narg < 3 )
      xprintf("SQLite driver: string list too short \n");

   /* get database name*/
   arg = (char *) mpl_tab_get_arg(dca, 2);

   /* Connect to database */
   rc = sqlite3_dlfuncs.open(arg, &sql->db);
   if (rc != SQLITE_OK)
   {
      xprintf("db_sqlite_open: Connect failed\n");
      xprintf("%s\n", sqlite3_dlfuncs.errstr(rc));
      xfree(sql);
      return NULL;
   }
   ...
   if ( sql->mode == 'W' )
   {
      rc =  sqlite3_dlfuncs.exec(sql->db, "BEGIN;", NULL, NULL, &errmsg);
      if (rc != SQLITE_OK)
      {
           xprintf("db_sqlite_open: Failed to start a transaction.\n");
           xprintf("%s\n", errmsg);
           sqlite3_dlfuncs.close(sql->db);
           xfree(sql);
           return NULL;
      }
   }
   ...
   {  query = (char *) sqllines[j];
      xprintf("%s\n", query);
      rc = sqlite3_dlfuncs.exec(sql->db, query, NULL, NULL, &errmsg);
      if (rc != SQLITE_OK)
      {
         xprintf("db_sqlite_open: Query\n\"%s\"\nfailed.\n", query);
         xprintf("%s\n", errmsg);
         sqlite3_dlfuncs.close(sql->db);
         xfree(sql);
         return NULL;
   ...
   {  sql->nf = mpl_tab_num_flds(dca);
      ...
      xprintf("%s\n", query);
      rc = sqlite3_dlfuncs.prepare(sql->db, query, -1, &sql->stmt, NULL);
      if(rc == SQLITE_OK)
      {
          rc = sqlite3_dlfuncs.step(sql->stmt);
      }
      if (!(rc == SQLITE_ROW || rc == SQLITE_DONE))
      {
         xprintf("db_sqlite_open: Query\n\"%s\"\nfailed.\n", query);
         xprintf("%s\n", sqlite3_dlfuncs.errmsg(sql->db));
         if(sql->stmt) sqlite3_dlfuncs.finalize(sql->stmt);
         sqlite3_dlfuncs.close(sql->db);
         xfree(query);
         xfree(sql);
         return NULL;
      ...
      {
         /* create references between query results and table fields*/
         total = sqlite3_dlfuncs.column_count(sql->stmt);
         if (total > SQL_FIELD_MAX)
         {  xprintf("db_sqlite_open: Too many fields (> %d) in query.\n"
               "\"%s\"\n", SQL_FIELD_MAX, query);
            xprintf("%s\n",sqlite3_dlfuncs.errmsg(sql->db));
            sqlite3_dlfuncs.finalize(sql->stmt);
            sqlite3_dlfuncs.close(sql->db);
            xfree(query);
            xfree(sql);
            return NULL;
         ...
         for (i = 1; i <= total; i++)
         {
            for (j = sql->nf; j >= 1; j--)
            {
               field = sqlite3_dlfuncs.column_name(sql->stmt, i-1);
               if (strcmp(mpl_tab_get_name(dca, j), field) == 0)
               break;
            }
            sql->ref[i] = j;
         }
         sqlite3_dlfuncs.reset(sql->stmt);
      }
      else
      {
         if(sqlite3_dlfuncs.column_count(sql->stmt) == 0)
            {
            xprintf("db_sqlite_open: Query was not a SELECT\n\"%s\"\n",
               query);
            xprintf("%s\n",sqlite3_dlfuncs.errmsg(sql->db));
            xfree(query);
            xfree(sql);
...
*  NAME
*
*  db_sqlite_open - open connection to ODBC data base
*
*  SYNOPSIS
*
*  #include "mplsql.h"
...
*  DESCRIPTION
*
*  The routine db_sqlite_open opens a connection to a SQLite data base.
*  It then executes the sql statements passed.
*
*  In the case of table read the SELECT statement is executed.
*
...
{  void  *ret;
   ...
   ret = db_sqlite_open_int(dca, mode, (const char **) sqllines);
   free_buffer(sqllines);
   return ret;
}

int db_sqlite_read(TABDCA *dca, void *link)
{  struct db_sqlite *sql;
   char            buf[SQL_FDLEN_MAX+1];
   ...
   sql = (struct db_sqlite *) link;

   xassert(sql != NULL);
   xassert(sql->mode == 'R');
   if (NULL == sql->stmt)
   {
      xprintf("db_sqlite_read: no result set available");
      return 1;
   }
   rc = sqlite3_dlfuncs.step(sql->stmt);
   if (rc == SQLITE_DONE) {
   ...
   if(rc != SQLITE_ROW) {
      xprintf("db_sqlite_read: %s\n",sqlite3_dlfuncs.errmsg(sql->db));
      return 1;
   }
   num_fields = sqlite3_dlfuncs.column_count(sql->stmt);
   for (i=1; i <= num_fields; i++)
   {
      ftype = sqlite3_dlfuncs.column_type(sql->stmt, i-1);
      switch(ftype)
      {
          case SQLITE_TEXT:
          case SQLITE_INTEGER:
          case SQLITE_FLOAT:
              field = sqlite3_dlfuncs.column_text(sql->stmt, i-1);
              strncpy(buf, (const char *) field, SQL_FDLEN_MAX);
              buf[SQL_FDLEN_MAX] = 0x00;
              if (ftype == SQLITE_TEXT) {
                if(sql->ref[i] > 0)
                    mpl_tab_set_str(dca, sql->ref[i], strtrim(buf));
                break;
              }
          ...
          case SQLITE_NULL:
              break;
          default:
              xprintf("can not deal with field type %d\n", ftype);
              return 1;
      }
   }
   return 0;
...
int db_sqlite_close(TABDCA *dca, void *link)
{
   ...
   if ( sql->mode == 'W' ) {
      xfree(sql->query);
      rc =  sqlite3_dlfuncs.exec(sql->db, "COMMIT;", NULL, NULL, &errmsg);
      if (rc != SQLITE_OK)
      {
           xprintf("db_sqlite_close: Failed to commit a transaction.\n");
           xprintf("%s\n", errmsg);
      }
   }
   if(sql->stmt != NULL) sqlite3_dlfuncs.finalize(sql->stmt);
   sqlite3_dlfuncs.close(sql->db);
   xfree(sql);
   dca->link = NULL;
   return rc;

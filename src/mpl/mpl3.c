      ...
      {  case O_MEMSET:
            ...
            {  ARG_LIST *e;
               ...
               if(!set) {
                    ...
                    if(!code->arg.set.list) code->arg.set.elemset = set; /* For now only if 1 dimension */
                    delete_tuple(mpl, temp);
               }
               /* check if the n-tuple is contained in the set array */
               temp = (set->dim != tuple_dimen(mpl, tuple)) ? build_subtuple(mpl, tuple, set->dim) : tuple;
...
{     mpl->stmt = stmt;
      ...
      {  case A_SET:
            if(mpl->gen_all) {
                xprintf("Generating set %s...", stmt->u.set->name,
                        mpl->show_delta ? "" : "\n");
                eval_whole_set_statement(mpl, stmt->u.set);
                if(mpl->show_delta) {
                    xprintf(", %d elements\n", get_size_set(mpl, stmt->u.set));
                    glp_show_mem_usage();
                }
            }
            break;
         case A_PARAMETER:
            if(mpl->gen_all) {
                xprintf("Generating param %s...", stmt->u.par->name,
                        mpl->show_delta ? "" : "\n");
                eval_whole_param_statemtnt(mpl, stmt->u.par);
                if(mpl->show_delta) {
                    xprintf(", %d elements\n", stmt->u.par->array->size);
                    glp_show_mem_usage();
                }
            }
            break;
         case A_VARIABLE:
            if(mpl->gen_all) {
                xprintf("Generating var %s...", stmt->u.var->name,
                        mpl->show_delta ? "" : "\n");
                eval_whole_var(mpl, stmt->u.var);
                if(mpl->show_delta) {
                    xprintf(", %d elements\n", stmt->u.var->array->size);
                    glp_show_mem_usage();
                }
            }
            break;

...
{     /* common storage area */
      ...
      glp_double *A_val; /* glp_double A_val[A_ptr[m+1]]; */
      ...
      /*--------------------------------------------------------------*/
      /* LP solution */
      glp_double *x; /* glp_double x[1+n]; */
      glp_double *y; /* glp_double y[1+m]; */
      glp_double *z; /* glp_double z[1+n]; */
      ...
      glp_double *S_val; /* glp_double S_val[S_ptr[m+1]]; */
      glp_double *S_diag; /* glp_double S_diag[1+m]; */
      ...
      glp_double *U_val; /* glp_double U_val[U_ptr[m+1]]; */
      glp_double *U_diag; /* glp_double U_diag[1+m]; */
      ...
      glp_double *phi_min; /* glp_double phi_min[1+ITER_MAX]; */
      ...
      /* iteration number, on which the value of phi reached its best
         (minimal) value */
      glp_double *best_x; /* glp_double best_x[1+n]; */
      glp_double *best_y; /* glp_double best_y[1+m]; */
      glp_double *best_z; /* glp_double best_z[1+n]; */
      /* best point (in the sense of the merit function phi) which has
         been reached on iteration iter_best */
      glp_double best_obj;
      /* objective value at the best point */
      glp_double *dx_aff; /* glp_double dx_aff[1+n]; */
      glp_double *dy_aff; /* glp_double dy_aff[1+m]; */
      glp_double *dz_aff; /* glp_double dz_aff[1+n]; */
      /* affine scaling direction */
      glp_double alfa_aff_p, alfa_aff_d;
      ...
      glp_double *dx_cc; /* glp_double dx_cc[1+n]; */
      glp_double *dy_cc; /* glp_double dy_cc[1+m]; */
      glp_double *dz_cc; /* glp_double dz_cc[1+n]; */
      /* centering corrector direction */
      glp_double *dx; /* glp_double dx[1+n]; */
      glp_double *dy; /* glp_double dy[1+m]; */
      glp_double *dz; /* glp_double dz[1+n]; */
      /* final combined direction dx = dx_aff+dx_cc, dy = dy_aff+dy_cc,
         dz = dz_aff+dz_cc */
      glp_double alfa_max_p;
      glp_double alfa_max_d;
...
{     int m = csa->m;
      ...
      csa->D = xcalloc(1+n, sizeof(glp_double));
      /* P := I */
      ...
      csa->S_ind = adat_symbolic(m, n, csa->P, csa->A_ptr, csa->A_ind,
         csa->S_ptr);
      csa->S_val = xcalloc(csa->S_ptr[m+1], sizeof(glp_double));
      csa->S_diag = xcalloc(1+m, sizeof(glp_double));
      ...
      csa->U_val = xcalloc(csa->U_ptr[m+1], sizeof(glp_double));
      csa->U_diag = xcalloc(1+m, sizeof(glp_double));
      csa->iter = 0;
      csa->obj = 0.0;
      csa->rpi = 0.0;
      csa->rdi = 0.0;
      ...
      csa->rmu = 0.0;
      csa->rmu0 = 0.0;
      csa->phi_min = xcalloc(1+ITER_MAX, sizeof(glp_double));
      csa->best_iter = 0;
      csa->best_x = xcalloc(1+n, sizeof(glp_double));
      csa->best_y = xcalloc(1+m, sizeof(glp_double));
      csa->best_z = xcalloc(1+n, sizeof(glp_double));
      csa->best_obj = 0.0;
      csa->dx_aff = xcalloc(1+n, sizeof(glp_double));
      csa->dy_aff = xcalloc(1+m, sizeof(glp_double));
      csa->dz_aff = xcalloc(1+n, sizeof(glp_double));
      csa->alfa_aff_p = 0.0;
      csa->alfa_aff_d = 0.0;
      csa->mu_aff = 0.0;
      csa->sigma = 0.0;
      csa->dx_cc = xcalloc(1+n, sizeof(glp_double));
      csa->dy_cc = xcalloc(1+m, sizeof(glp_double));
      csa->dz_cc = xcalloc(1+n, sizeof(glp_double));
      csa->dx = csa->dx_aff;
      csa->dy = csa->dy_aff;
      csa->dz = csa->dz_aff;
      csa->alfa_max_p = 0.0;
...
static void A_by_vec(struct csa *csa, glp_double x[], glp_double y[])
{     /* compute y = A*x */
      int m = csa->m;
      int *A_ptr = csa->A_ptr;
      int *A_ind = csa->A_ind;
      glp_double *A_val = csa->A_val;
      int i, t, beg, end;
      glp_double temp;
      for (i = 1; i <= m; i++)
      {  temp = 0.0;
...
static void AT_by_vec(struct csa *csa, glp_double x[], glp_double y[])
...
static int solve_NE(struct csa *csa, glp_double y[])
{     int m = csa->m;
      int n = csa->n;
      int *P = csa->P;
      int i, j, ret = 0;
      ...
      h = xcalloc(1+m, sizeof(glp_double));
      ...
      /* w := P*h */
      w = xcalloc(1+m, sizeof(glp_double));
      ...
      r = xcalloc(1+m, sizeof(glp_double));
      /* w := A'*y */
      w = xcalloc(1+n, sizeof(glp_double));
      AT_by_vec(csa, y, w);
      /* w := D*w */
...
static int solve_NS(struct csa *csa, glp_double p[], glp_double q[], glp_double r[],
      glp_double dx[], glp_double dy[], glp_double dz[])
{     int m = csa->m;
      int n = csa->n;
      glp_double *x = csa->x;
      glp_double *z = csa->z;
...
{     int m = csa->m;
      ...
      glp_double *z = csa->z;
      int i, j;
      glp_double norm1, bnorm, norm2, cnorm, cx, by, *work, temp;
      /* compute value of the objective function */
      temp = c[0];
      for (j = 1; j <= n; j++) temp += c[j] * x[j];
      csa->obj = temp;
      /* norm1 = ||A*x-b|| */
      work = xcalloc(1+m, sizeof(glp_double));
      A_by_vec(csa, x, work);
      norm1 = 0.0;
      for (i = 1; i <= m; i++)
...
{     int m = csa->m;
      ...
      glp_double *y = csa->y;
      glp_double *z = csa->z;
      glp_double *dx_aff = csa->dx_aff;
      glp_double *dy_aff = csa->dy_aff;
      glp_double *dz_aff = csa->dz_aff;
      glp_double *dx_cc = csa->dx_cc;
      glp_double *dy_cc = csa->dy_cc;
      glp_double *dz_cc = csa->dz_cc;
      glp_double *dx = csa->dx;
      glp_double *dy = csa->dy;
      glp_double *dz = csa->dz;
      int i, j, ret = 0;
      glp_double temp, gamma_p, gamma_d, *p, *q, *r;
      /* allocate working arrays */
      p = xcalloc(1+m, sizeof(glp_double));
      q = xcalloc(1+n, sizeof(glp_double));
      r = xcalloc(1+n, sizeof(glp_double));
      /* p = b - A*x */
      A_by_vec(csa, x, p);
...
{     struct csa _dsa, *csa = &_dsa;
      ...
      glp_double dir, *A_val, *b, *c, *x, *y, *z;
      xassert(m > 0);
      xassert(n > 0);
      /* allocate working arrays */
      A_ptr = xcalloc(1+m+1, sizeof(int));
      A_ind = xcalloc(1+nnz, sizeof(int));
      A_val = xcalloc(1+nnz, sizeof(glp_double));
      b = xcalloc(1+m, sizeof(glp_double));
      c = xcalloc(1+n, sizeof(glp_double));
      x = xcalloc(1+n, sizeof(glp_double));
      y = xcalloc(1+m, sizeof(glp_double));
      z = xcalloc(1+n, sizeof(glp_double));
